...
{
    public partial class PlanoDeAcaoAdmin : BaseAutPage
    {
        ...
        private void InterfaceUpdate(decimal pPLNAC_ID)
        {
            try
            {
                DataFieldCollection lFields = new DataFieldCollection();
                ...
                lFields.Add(NC_PlanoAcaoQD._MATRICULA, ddlFuncionario.SelectedValue);
                lFields.Add(NC_PlanoAcaoQD._STPLNAC_ID, ddlSTPLNAC_ID.SelectedValue);
                lFields.Add(NC_PlanoAcaoQD._PLNAC_REGDATE, DateTime.Now);
                lFields.Add(NC_PlanoAcaoQD._PLNAC_REGUSER, ((LoginUserDo)Session["_SessionUser"]).MATRICULA);
                lFields.Add(NC_PlanoAcaoQD._PLNAC_STATUS, LocalInstance.StatusAtivo);


                lReturn = NC_PlanoAcaoDo.Update(lFields, LocalInstance.ConnectionInfo);
        ...
        private void InterfaceDeleteAcao(decimal pACS_ID, string pACS_STATUS)
        {
            try
            {
                ...
                lFieldsAcoes.Add(NC_AcoesQD._ACS_SITUACAO, (decimal)SituacaoAcao.Cancelado);
                lFieldsAcoes.Add(NC_AcoesQD._ACS_REGDATE, DateTime.Now);
                lFieldsAcoes.Add(NC_AcoesQD._ACS_REGUSER, ((LoginUserDo)Session["_SessionUser"]).MATRICULA);
        ...
        private void InterfaceUpdateExecutarAcao(decimal pACS_ID, decimal pACS_SITUACAO)
        {
            try
            {
                DataFieldCollection lFieldsAcoes = new DataFieldCollection();
        ...
        private void LoadSituacaoPlano()
        {
            DataTable ltable = NC_PlanoAcaoDo.GetAllNC_SituacaoPlanoAcao(LocalInstance.ConnectionInfo);
            ddlSTPLNAC_ID.DataSource = ltable;
            ddlSTPLNAC_ID.DataTextField = "STPLNAC_DESCRICAO";
            ddlSTPLNAC_ID.DataValueField = "STPLNAC_ID";
            ddlSTPLNAC_ID.DataBind();
            ddlSTPLNAC_ID.Items.Insert(0, new ListItem("--Selecione--", "0"));
        ...
        private void LoadPlanoDeAcao(decimal pPLNAC_ID)
        {
            ...
            if (lTable.Rows.Count > 0)
            {
                ...
                ddlSTPLNAC_ID.SelectedValue  = lTable.Rows[0][NC_PlanoAcaoQD._STPLNAC_ID.Name].ToString();
                LoadAcoes();
            }
        }
        ...
        private void LoadAcoes(decimal pACS_ID)
        {
            ...
            if (lTable.Rows.Count > 0)
            {
                ...
                if (lTable.Rows[iIndice][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() == (decimal)SituacaoAcao.Concluído)
                {
                    ...
                    txtACS_DATAEXECUCAO.Enabled = true;
                ...
                if (e.CommandName != "Page") //Paginação                                                                          
                {
                    ...
                    else if (e.CommandName == "Alterar")
                    {
                        ...
                        if (lTable.Rows.Count > 0)
                        {
                            ...
                            if (lTable.Rows[iIndice][NC_AcoesQD._ACS_DATAEXECUCAO.Name].ToString() != "")
                            {
                                txtACS_DATAEXECUCAO.Text = lTable.Rows[iIndice][NC_AcoesQD._ACS_DATAEXECUCAO.Name].DBToDateTime().ToShortDateString();
                                txtACS_DESCRICAOEXECUCAO.Text = lTable.Rows[iIndice][NC_AcoesQD._ACS_DESCRICAOEXECUCAO.Name].ToString();
                                divExecutarAcao.Visible = true;
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    switch (int.Parse(DataBinder.Eval(e.Row.DataItem, "ACS_SITUACAO").ToString()))
                    {
                        ...
                        case (int)SituacaoAcao.Atrasado:
                            ...
                            {
                                //Se for a última repragrama senão pode editar a data fim
                                if (DataBinder.Eval(e.Row.DataItem, "ACS_DATATERMINOPREVISAO").DBToDateTime() < lblDataConclusao.Text.DBToDateTime())
                                    e.Row.Cells[7].Enabled = true;
                                else
        ...
        protected void btnOkAcoes_Click(object sender, EventArgs e)
        {
            ...
            {
                InterfaceUpdateAcao(hidACS_ID.Value.DBToDecimal(), ddlACS_SITUACAO.SelectedValue.DBToDecimal());
                /*
        ...
        protected void btnVoltar_Click(object sender, EventArgs e)
        {
            if (((LoginUserDo)Session["_SessionUser"]).LoginName == "ADMIN.GNC")
                Response.Redirect("RegistroNaoConformidadeAdmin.aspx?OCR_ID=" + hidOCR_ID.Value, true);
            else
        ...
        {
            InterfaceUpdateExecutarAcao(hidACS_ID.Value.DBToDecimal(), (decimal)SituacaoAcao.Concluído);
        }
        ...
        protected void btnOkPlano_Click(object sender, EventArgs e)
        {
            if (hidPLNAC_ID.Value != "")
                InterfaceUpdate(hidPLNAC_ID.Value.DBToDecimal());
        }
}
}