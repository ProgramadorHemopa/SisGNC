...
namespace HMP.WebInterface.SisRNCWeb.Www.Pages
{
    public partial class PlanoDeAcao : BaseAutPage
    {
        ...
        {
            DataTable ltable = NC_OcorrenciaDo.GetAllFuncionariosByMatricula(0, LocalInstance.ConnectionInfo);
        ...
        private void Clear()
        {
            ...
            txtNovaDataFim.Text = "";
            txtACS_JUSTIFICATIVAATRASO.Text = "";
        }
        ...
        private bool ValidarCancelaPA()
        {
            bool lValido = true;
            string lMensagem = "";

            if (rblCancelamento_Situacao.SelectedValue.DBToDecimal() == 0)
            {
                lMensagem += "Selecione uma opção";
                lValido = false;
            }
            if (txtJustificativaNQ.Text.ToString() == "")
            {
                lMensagem += "Insire a Justificativa do Cancelamento";
                lValido = false;
            }

            if (!lValido)
            {
                MessageBox1.wuc_ShowMessage("Cancelamento não concluído. Informe os seguintes campos: <br/>" + lMensagem, 3);
            }
        ...
        private bool ValidarInsertAcoes()
        {
            ...
            if (txtACS_DATAINICIOPREVISAO.Text != "" && txtACS_DATATERMINOPREVISAO.Text != "")
            {
                ...
                {
                    if (hidSTPLNAC_ID.Value.DBToDecimal() != (decimal)SituacaoPlanoAcaoFLUXO.EmElaboracao && txtACS_DATATERMINOPREVISAO.Text.DBToDateTime() > lblDataConclusao.Text.DBToDateTime())
                    {
        ...
        private bool ValidarInsertExecucao()
        {
            ...
            if (divJustificaAtraso.Visible)
            {
                if (txtACS_JUSTIFICATIVAATRASO.Text == "")
                {
                    lMensagem += "-Justificativa Atraso <br/>";
                    lValido = false;
                }
            }
        ...
        private bool ValidarUltimaAcao()
        {
            /* bool lValido = true;
             ...
             if (lTable.Rows.Count > 0)
             {
                 for (int i = 0; i < lTable.Rows.Count; i++)
                 {
                     if (lTable.Rows[i][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() != (decimal)SituacaoAcao.ConcluídoJustificado && lTable.Rows[i][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() != (decimal)SituacaoAcao.Atrasado && lTable.Rows[i][NC_AcoesQD._ACS_DATATERMINOPREVISAO.Name].DBToDateTime() == lblDataConclusao.Text.DBToDateTime() || lTable.Rows[i][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() != (decimal)SituacaoAcao.Concluído && lTable.Rows[i][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() == (decimal)SituacaoAcao.DevolvidoReprogramacao && lTable.Rows[i][NC_AcoesQD._ACS_DATATERMINOPREVISAO.Name].DBToDateTime() == lblDataConclusao.Text.DBToDateTime())
                         lValido = false;
                 }
            ...
            if (lTable.Rows.Count > 0)
            {
                for (int i = 0; i < lTable.Rows.Count; i++)
                {
                    ...
                    if (lTable.Rows[i][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() == (decimal)SituacaoAcao.Concluído || lTable.Rows[i][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() == (decimal)SituacaoAcao.ConcluídoJustificado || lTable.Rows[i][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() == (decimal)SituacaoAcao.EmExecução && lTable.Rows[i][NC_AcoesQD._ACS_DATATERMINOPREVISAO.Name].DBToDateTime() == lblDataConclusao.Text.DBToDateTime() && !lValido)
                        lValido = false;
        ...
        private void InterfaceInclude()
        {
            try
            {
                ...
                lFields.Add(NC_PlanoAcaoQD._OCR_ID, hidOCR_ID.Value.DBToDecimal());
                ...
                lFields.Add(NC_PlanoAcaoQD._STPLNAC_ID, (decimal)SituacaoPlanoAcaoFLUXO.EmElaboracao);
                ...
                else
                {
                    ...
                    hidSTPLNAC_ID.Value = ((decimal)SituacaoPlanoAcaoFLUXO.EmElaboracao).ToString();
                    lblSTPLNAC_ID.Text = "Em Elaboração";
                }
            }
        ...
        private void InterfaceUpdateAcoesPlano(decimal pPLNAC_ID, decimal pOCR_ID)
        {
            try
            {
                ...
                lFieldsPlanoAcao.Add(NC_PlanoAcaoQD._STPLNAC_ID, (decimal)SituacaoPlanoAcaoFLUXO.EmExecucao);
                ...
                // lReturn = NC_AcoesDo.UpdateSituacaoAcoes(lFields, lFieldsPlanoAcao, LocalInstance.ConnectionInfo);
                ...
                else
                {
                    ...
                    hidSTCOCR_ID.Value = ((decimal)SituacaoPlanoAcaoFLUXO.EmExecucao).ToString();
                    LoadSituacaoPlano(((decimal)SituacaoPlanoAcaoFLUXO.EmExecucao).ToString());
        ...
        private void InterfaceCancelaPlano(decimal pPLNAC_ID, decimal pOCR_ID)
        {
            try
            {
                DataFieldCollection lFields = new DataFieldCollection();
                ...
                //Atualizar Situação da Ocorrencia 
                lFieldsOcorrencia.Add(NC_OcorrenciaQD._OCR_ID, pOCR_ID);
                if (rblCancelamento_Situacao.SelectedValue.DBToDecimal() == 1)
                {
                    lFieldsOcorrencia.Add(NC_OcorrenciaQD._STCOCR_ID, (decimal)SituacaoOcorrencia.Cancelado);
                }
                else if (rblCancelamento_Situacao.SelectedValue.DBToDecimal() == 2)
                {
                ...
                lFieldsPlanoAcao.Add(NC_PlanoAcaoQD._PLNAC_ID, pPLNAC_ID);
                // lFieldsPlanoAcao.Add(NC_PlanoAcaoQD._PLNAC_SITUACAO, (decimal)SituacaoPlanoAcao.EmExecucao);
                lFieldsPlanoAcao.Add(NC_PlanoAcaoQD._STPLNAC_ID, (decimal)SituacaoPlanoAcaoFLUXO.Cancelado);
                lFieldsPlanoAcao.Add(NC_PlanoAcaoQD._PLNAC_DATAJUSTIFICATIVANQ, DateTime.Now);
                lFieldsPlanoAcao.Add(NC_PlanoAcaoQD._PLNAC_JUSTIFICATIVANQ, txtJustificativaNQ.Text);
                lFieldsPlanoAcao.Add(NC_PlanoAcaoQD._PLNAC_REGDATE, DateTime.Now);
                lFieldsPlanoAcao.Add(NC_PlanoAcaoQD._PLNAC_REGUSER, ((LoginUserDo)Session["_SessionUser"]).MATRICULA);
                lFieldsPlanoAcao.Add(NC_PlanoAcaoQD._PLNAC_STATUS, LocalInstance.StatusAtivo);
                ...
                //Atualiza todas ações para EmExecução
                lFields.Add(NC_AcoesQD._PLNAC_ID, pPLNAC_ID);
                lFields.Add(NC_AcoesQD._ACS_SITUACAO, (decimal)SituacaoAcao.Cancelado);
                lFields.Add(NC_AcoesQD._ACS_REGDATE, DateTime.Now);
                lFields.Add(NC_AcoesQD._ACS_REGUSER, ((LoginUserDo)Session["_SessionUser"]).MATRICULA);
                lFields.Add(NC_AcoesQD._ACS_STATUS, LocalInstance.StatusAtivo);
                ...
                {
                    MessageBox1.wuc_ShowMessage("Plano de Ação cancelado com sucesso.", "ConsultaRNC.aspx?OCR_ID=" + pOCR_ID, 1);
        ...
        private void InterfaceUpdateAcao(decimal pACS_ID, decimal pACS_SITUACAO)
        {
            try
            {
                ...
                if (pACS_SITUACAO == (decimal)SituacaoAcao.NãoIniciada && !ValidarInsertAcoes())
                    return;
                else if (pACS_SITUACAO == (decimal)SituacaoAcao.Concluído && !ValidarInsertExecucao() || pACS_SITUACAO == (decimal)SituacaoAcao.ConcluídoJustificado && !ValidarInsertExecucao())
                    return;
                ...
                if (pACS_SITUACAO == (decimal)SituacaoAcao.Concluído || pACS_SITUACAO == (decimal)SituacaoAcao.ConcluídoJustificado)
                {
                    ...
                    if (pACS_SITUACAO == (decimal)SituacaoAcao.ConcluídoJustificado)
                    {
                        lFieldsAcoes.Add(NC_AcoesQD._ACS_JUSTIFICATIVAATRASO, txtACS_JUSTIFICATIVAATRASO.Text);
                    ...
                    lFieldsPlanoAcao.Add(NC_PlanoAcaoQD._STPLNAC_ID, (decimal)SituacaoPlanoAcaoFLUXO.EmVerificacaoEficacia);
                    lFieldsPlanoAcao.Add(NC_PlanoAcaoQD._PLNAC_REGDATE, DateTime.Now);
                    lFieldsPlanoAcao.Add(NC_PlanoAcaoQD._PLNAC_REGUSER, ((LoginUserDo)Session["_SessionUser"]).MATRICULA);
                    lFieldsPlanoAcao.Add(NC_PlanoAcaoQD._PLNAC_STATUS, LocalInstance.StatusAtivo);
                ...
                {
                    if (int.Parse(hidSTPLNAC_ID.Value) != (decimal)SituacaoPlanoAcaoFLUXO.EmElaboracao && txtACS_DATATERMINOPREVISAO.Text.DBToDateTime() > lblDataConclusao.Text.DBToDateTime())
                    {
                    ...
                    lFieldsAcoes.Add(NC_AcoesQD._UNIDADES_QUEM, ddlUNIDADES_QUEM.SelectedValue.DBToDecimal());
                ...
                else
                {
                    ...
                    LoadPlanoDeAcao(hidPLNAC_ID.Value.DBToDecimal());
                    LoadSituacaoPlano(hidSTPLNAC_ID.Value.ToString());
        ...
        private void InterfaceIncludeAcoes()
        {
            try
            {
                ...
                if ((int)SituacaoPlanoAcaoFLUXO.EmElaboracao == int.Parse(hidSTPLNAC_ID.Value))
                ...
                lFields.Add(NC_AcoesQD._ACS_QUANTO, txtACS_QUANTO.Text.ToString());
                lFields.Add(NC_AcoesQD._UNIDADES_QUEM, ddlUNIDADES_QUEM.SelectedValue.DBToDecimal());
                lFields.Add(NC_AcoesQD._ACS_REGDATE, DateTime.Now);
                lFields.Add(NC_AcoesQD._ACS_REGUSER, ((LoginUserDo)Session["_SessionUser"]).MATRICULA);
                lFields.Add(NC_AcoesQD._ACS_STATUS, LocalInstance.StatusAtivo);
                ...
                else
                {
                    ...
                    else if ((int)SituacaoPlanoAcaoFLUXO.EmExecucao == int.Parse(hidSTPLNAC_ID.Value))
        ...
        private void InterfaceIncludeVerificacaoEficacia(decimal pPLNAC_ID)
        {
            try
            {
                ...
                if (rblVRFEFC_SITUACAO.SelectedValue == "1")//Concluida
                    ...
                    lFieldsOcorrencia.Add(NC_OcorrenciaQD._STCOCR_ID, (decimal)SituacaoOcorrencia.EmElaboracaoPlanoAcao);
                else if (rblVRFEFC_SITUACAO.SelectedValue == "3")//Nao aprovado - finaliza Ocorrencia
                    lFieldsOcorrencia.Add(NC_OcorrenciaQD._STCOCR_ID, (decimal)SituacaoOcorrencia.Concluida);
                ...
                lFieldsAcoes.Add(NC_AcoesQD._PLNAC_ID, pPLNAC_ID);
                if (rblVRFEFC_SITUACAO.SelectedValue == "1")
                    lFieldsAcoes.Add(NC_AcoesQD._ACS_SITUACAO, (decimal)SituacaoAcao.Concluído);
                else if (rblVRFEFC_SITUACAO.SelectedValue == "2")
                ...
                else
                {
                    if (rblVRFEFC_SITUACAO.SelectedValue == "1")//Aprovada
                        ...
                        EnviaMensagemEmail(1, "Gestão de Não Conformidade", " Foi Reprovada. \r\r Elabore um novo Plano de Ação para a RNC. \r\r\r Acesse http://10.95.3.2/SisGNCWeb/Aut/Page/ConsultaRNC.aspx?OCR_ID=" + hidOCR_ID.Value, "");
                    else if (rblVRFEFC_SITUACAO.SelectedValue == "3")//Cancelada
                        EnviaMensagemEmail(1, "Gestão de Não Conformidade", " Foi Cancelada. \r\r \r\r\r Acesse http://10.95.3.2/SisGNCWeb/Aut/Page/ConsultaRNC.aspx?OCR_ID=" + hidOCR_ID.Value, "");
        ...
        private void LoadDadosOcorrencia(decimal pOCR_ID)
        {
            try
            {
                ...
                if (lTable.Rows.Count > 0)
                {
                    ...
                    //LoadSituacaoOcorrencia(lTable.Rows[0][NC_OcorrenciaQD._STCOCR_ID.Name].ToString());
                }
                else
                {
        ...
        private void LoadPlanoDeAcao(decimal pPLNAC_ID)
        {
            ...
            if (lTable.Rows.Count > 0)
            {
                ...
                if (lTable.Rows[0][NC_PlanoAcaoQD._PLNAC_JUSTIFICATIVANQ.Name].ToString() != "")
                {
                    txtJUST_DESCRICAO.Text = lTable.Rows[0][NC_PlanoAcaoQD._PLNAC_JUSTIFICATIVANQ.Name].ToString();
                    divJustCancelamentoPA.Visible = true;
                }


                if (lTable.Rows[0]["DATA_PREVISAO_CONCLUSAO"].DBToDateTime().ToShortDateString() != "" || lTable.Rows[0]["DATA_PREVISAO_CONCLUSAO"].DBToDateTime().ToShortDateString() != "01/01/0001")
                    lblDataConclusao.Text = lTable.Rows[0]["DATA_PREVISAO_CONCLUSAO"].DBToDateTime().ToShortDateString();


                hidSTPLNAC_ID.Value = lTable.Rows[0][NC_PlanoAcaoQD._STPLNAC_ID.Name].ToString();
        ...
        private void LoadSituacaoPlano(string pSTPLNAC_ID)
        {
            switch (pSTPLNAC_ID)
            {
                case "1":
                    ...
                    if (((LoginUserDo)Session["_SessionUser"]).UNIDADE_ID == 60) //NQ
                    {
                        btnCancelarPA.Visible = true;
                    }

                    break;
                case "2":
                    lblSTPLNAC_ID.Text = "Em Execução";
                    btnExcluir.Visible = false;
                    btnNovaAcao.Visible = false;
                    btnExcluir.Visible = false;
                    btnConcluirPlano.Visible = false;
                    ...
                    break;
                case "6":
                    lblSTPLNAC_ID.Text = "Em Verificação de Eficácia";
                    btnExcluir.Visible = false;
                    btnNovaAcao.Visible = false;
                    btnExcluir.Visible = false;
                    btnConcluirPlano.Visible = false;
                    if (((LoginUserDo)Session["_SessionUser"]).UNIDADE_ID == 60) //NQ
                    {
                        btnVerificaEficacia.Visible = true;
                        btnCancelarPA.Visible = true;
                    ...
                    break;
                case "4":
                    lblSTPLNAC_ID.Text = "Concluído - Eficaz";
                    btnExcluir.Visible = false;
                    btnNovaAcao.Visible = false;
                    btnExcluir.Visible = false;
                    btnConcluirPlano.Visible = false;
                ...
                case "5":
                    lblSTPLNAC_ID.Text = "Concluído - Não Eficaz";
                    btnExcluir.Visible = false;
                    btnNovaAcao.Visible = false;
                    btnExcluir.Visible = false;
                    btnConcluirPlano.Visible = false;
        ...
        //Verifica se existe ação atrasada mais de 2x e disponibiliza a verificação da eficácia para o NQ
        private void LoadAcoesEficacia()
        {

            DataTable lTableAcoesEficacia;
            lTableAcoesEficacia = NC_ReprogramacaoAcoesDo.GetNC_ReprogramacaoAcoesByACS_Id_Eficacia(hidACS_ID_MAIORATRASADO.Value.DBToDecimal(), LocalInstance.ConnectionInfo);

            if (lTableAcoesEficacia.Rows.Count >= 2 && lblDataConclusao.Text.DBToDateTime() < DateTime.Today.DBToDateTime())
            {
                btnVerificaEficacia.Visible = true;
            }
            ...
            if (lTable.Rows.Count > 0)
            {
                ...
                if (pACAOGRD == "Reprogramar")
                {
                    ...
                    else if (pACS_SITUACAO == (decimal)SituacaoAcao.Atrasado)
                    {
                        ...
                        divReprogramacao.Visible = true;
                        if (lTable.Rows.Count >= 2)
                        {
                            btnNovaReprogramacao.Visible = false;
                            divReprogramacao.Visible = true;
                            /*
                                if (((LoginUserDo)Session["_SessionUser"]).UNIDADE_ID == 60) // Se NQ Libera a Analise de Eficacia
                                {
                                    btnVerificaEficacia.Visible = true;
                                }
                                */
                ...
                else if (pACAOGRD == "Visualizar" && ((LoginUserDo)Session["_SessionUser"]).UNIDADE_ID == 60) // Se NQ
                {
                    ...
                    {
                        btnVerificaEficacia.Visible = false;
                ...
                if (e.CommandName != "Page") //Paginação                                                                          
                {
                    ...
                    if (e.CommandName == "Visualizar")
                    {
                        ...
                        if (lTable.Rows.Count > 0)
                        {
                            ...
                            if (lTable.Rows[iIndice][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() == (decimal)SituacaoAcao.Concluído || lTable.Rows[iIndice][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() == (decimal)SituacaoAcao.ConcluídoJustificado)
                            {
                                txtACS_DATAEXECUCAO.Text = lTable.Rows[iIndice][NC_AcoesQD._ACS_DATAEXECUCAO.Name].DBToDateTime().ToShortDateString();
                                txtACS_DESCRICAOEXECUCAO.Text = lTable.Rows[iIndice][NC_AcoesQD._ACS_DESCRICAOEXECUCAO.Name].ToString();
                                divExecutarAcao.Visible = true;
                                if (lTable.Rows[iIndice][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() == (decimal)SituacaoAcao.ConcluídoJustificado)
                                {
                                    txtACS_JUSTIFICATIVAATRASO.Text = lTable.Rows[iIndice][NC_AcoesQD._ACS_JUSTIFICATIVAATRASO.Name].ToString();
                                    divJustificaAtraso.Visible = true;
                                    txtACS_JUSTIFICATIVAATRASO.Enabled = false;
                                    divAlertaAcaoAtrasada.Visible = false;
                                }
                                else
                                {
                                    divJustificaAtraso.Visible = false;
                                }
                    ...
                    else if (e.CommandName == "Executar")
                    {
                        ...
                        if (lTable.Rows.Count > 0)
                        {
                            if (hidOCR_NUMEROANTIGO.Value == "" && lTable.Rows[iIndice][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() == (decimal)SituacaoAcao.Atrasado && DateTime.Today.DBToDateTime() > lblDataConclusao.Text.DBToDateTime())
                            {
                            ...
                            /*  if (hidOCR_NUMEROANTIGO.Value == "" && lTable.Rows[iIndice][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() == (decimal)SituacaoAcao.EmExecução && ValidarUltimaAcao() || lTable.Rows[iIndice][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() == (decimal)SituacaoAcao.DevolvidoReprogramacao && ValidarUltimaAcao())
                              {
                                  MessageBox1.wuc_ShowMessage("Ação Negada. O Responsável pela Elaboração do Plano de Ação deve realizar todas as ações intermediárias para executar a ação de conclusão do PA.", 2);
                                  return;
                              }
                              */


                            if (lTable.Rows[iIndice][NC_AcoesQD._ACS_SITUACAO.Name].DBToDecimal() == (decimal)SituacaoAcao.Atrasado)
                            {
                                divAlertaAcaoAtrasada.Visible = true;
                                divJustificaAtraso.Visible = true;
                            }
                            else
                            {
                    ...
                    */
                    ...
                    switch (int.Parse(DataBinder.Eval(e.Row.DataItem, "ACS_SITUACAO").ToString()))
                    {
                        ...
                        case (int)SituacaoAcao.Atrasado:
                            ...
                            if (ddlFuncionario.SelectedValue.DBToDecimal() == ((LoginUserDo)Session["_SessionUser"]).MATRICULA || DataBinder.Eval(e.Row.DataItem, "UNIDADES_QUEM").DBToDecimal() == ((LoginUserDo)Session["_SessionUser"]).UNIDADE_ID)//Se tiver atrasado o Resp. pelo PA pode solicitar a reprogramação
                            {
                                //Se for a última repragrama senão pode editar a data fim - Ricardo 
                                // Se estiver atrasada e data termino menor que a data de conclusão do PA pode executar a ação - Cleiton - 20/12/17 
                                if (DateTime.Today.DBToDateTime() > lblDataConclusao.Text.DBToDateTime())
                                {
                                    e.Row.Cells[7].Enabled = false;
                                    e.Row.Cells[9].Enabled = false;
                                    e.Row.Cells[10].Enabled = false;

                                    if (DataBinder.Eval(e.Row.DataItem, "ACS_ID").DBToDecimal() == hidACS_ID_MAIORATRASADO.Value.DBToDecimal() && ValidarReprogramacaoAcoesPA())
                                    {
                        ...
                        case (int)SituacaoAcao.ConcluídoJustificado:
                        ...
                        case (int)SituacaoAcao.DevolvidoReprogramacao:
                            ...
                            if (ddlFuncionario.SelectedValue.DBToDecimal() == ((LoginUserDo)Session["_SessionUser"]).MATRICULA)//Se tiver atrasado o Resp. pelo PA pode solicitar a reprogramação
                                ...
                                e.Row.Cells[10].Enabled = false;
                            break;
                        case (int)SituacaoAcao.Ineficaz:
        ...
        protected void btnOkAcoes_Click(object sender, EventArgs e)
        {
            ...
            {
                if (hidSTPLNAC_ID.Value == SituacaoPlanoAcaoFLUXO.EmElaboracao.ToString())
        ...
        protected void btnCancelarAcoes_Click(object sender, EventArgs e)
        {
            ...
            divAcoes.Visible = false;
            divExecutarAcao.Visible = false;
            if (((LoginUserDo)Session["_SessionUser"]).UNIDADE_ID == 60 && hidSTPLNAC_ID.Value.DBToDecimal() == (decimal)SituacaoPlanoAcaoFLUXO.EmVerificacaoEficacia)//NQ e Verificação Eficacia
            {
                btnVerificaEficacia.Visible = true;
            }
            else
        ...
        {
            if (divJustificaAtraso.Visible) //campo para justificativa de ação atrasada
                InterfaceUpdateAcao(hidACS_ID.Value.DBToDecimal(), (decimal)SituacaoAcao.ConcluídoJustificado);
            else
        ...
        protected void btnVerificaEficacia_Click(object sender, EventArgs e)
        {
            ...
            txtPLNAC_NOME.Text = txtPLAC_NOME.Text;
            txtPLNAC_NOME.Enabled = false;
            SetFocus(txtVRFEFC_OBSERVACAO);
            btnVerificaEficacia.Visible = false;
        }
        ...
        protected void btnCancelarVerificacao_Click(object sender, EventArgs e)
        {
            ...
            btnVerificaEficacia.Visible = true;
            SetFocus(grdReprogramacao);
        }

        protected void btnCancelarPA_Click(object sender, EventArgs e)
        {
            divCancelarPA.Visible = true;
            txtDataJustificativaNQ.Text = DateTime.Now.ToShortDateString();
            SetFocus(txtJustificativaNQ);
            btnCancelarPA.Visible = false;
            btnOkCancelaPA.Visible = true;
            btnFechaCancelaPA.Visible = true;
        }

        protected void btnOkCancelaPA_Click(object sender, EventArgs e)
        {
            InterfaceCancelaPlano(hidPLNAC_ID.Value.DBToDecimal(), hidOCR_ID.Value.DBToDecimal());

        }

        protected void btnFechaCancelaPA_Click(object sender, EventArgs e)
        {
            txtJustificativaNQ.Text = "";
            btnOkCancelaPA.Visible = false;
            btnFechaCancelaPA.Visible = false;
            divCancelarPA.Visible = false;
            btnCancelarPA.Visible = true;
        }
    }
}